    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                //update by wangwj at 16-3-4 for match uploadFileApi
                try {
                    var $file = $('#' + file.id);
                    var fileName = file.name;
                    if (ret.status == 'success') {
                        var serverUrl = editor.getOpt('imageUrlPrefix');
                        var registerUrl = serverUrl + "/storageRecord/register";
                        var postData = {
                            "fileName": fileName,
                            "filePath": ret.data.filePath,
                            "normalization": "sync"
                        };
                        dhcAjax({
                            url: registerUrl,              //请求地址
                            type: "POST",                       //请求方式
                            data: postData,        //请求参数
                            dataType: "json",
                            success: function (response, xml) {
                                ...
                                if (result.status == 'success') {
                                    var link = "/download/" + result.data.id + "?type=standard";
                                    var json = {
                                        name: fileName,
                                        original: fileName,
                                        size: file.size,
                                    ...
                                    $file.find('.error').text("failure").show();
                                }
                            },
