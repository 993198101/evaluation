    ...
    if (scrawlObj.isScrawl) {
        ...
        if (!!base64) {
            ...
            dhcAjax({
                url: actionUrl,              //请求地址
                type: "POST",                       //请求方式
                data: fileUploadData,        //请求参数
                dataType: "json",
                success: function (response, xml) {
                    var result = JSON.parse(response);
                    if(result.status == 'success'){
                        var filePath = result.data.filePath;
                        var serverUrl = editor.getOpt('scrawlUrlPrefix');
                        ...
                        dhcAjax({
                            ...
                            success: function (response, xml) {
                                ...
                                if(result.status == 'success'){
                                    var imgObj = {},
                                        url = editor.options.scrawlUrlPrefix +"/download/" + result.data.id;
                                    imgObj.src = url;
                                    imgObj._src = url;
                                    imgObj.alt = "涂鸦" + new Date().getTime() || '';
                                    imgObj.title = "涂鸦" + new Date().getTime() || '';
                                    editor.execCommand("insertImage", imgObj);
                                    dialog.close();
                                }else{
                                    alert(result.status);
